...
{foreach from=$edit_validity item=validity_item}
							<input type="radio" id="id_radio_edit_{$validity_item.name|lower}" name="{$validity_item.name}" value="{$validity_item.value}" {$validity_item.checked}>&nbsp;{$validity_item.itemname}</input>&nbsp;&nbsp;
{/foreach}
						</td>
					</tr>
{* 基本プロフィール *}
...
{/foreach}
					...
					<tr class = "c_tr_edit">
						...
						<td class="c_td_edit_value_3col" colspan=3 id="id_td_edit_chngpassword">
							{$passwd_mess1}<input class="c_table_td_textval" id="id_txt_edit_password" type="password" name="PASSWORD_OLD" value="{$edit_password}" title=""></input>
							</br>
