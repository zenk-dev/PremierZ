...
// ==================================
	...
	$l_get_key2			= "BASE_ID";							// 認証で取得するキー項目2
	$l_del_workplace_id			= "";
...
/*----------------------------------------------------------------------------
  POST変数取得
  ----------------------------------------------------------------------------*/
	$l_del_workplace_id = null;
	$i=0;
	// チェックが入った作業場所IDを取得
	foreach($_POST as $key => $value){
		if(mb_strpos($key,"nm_workplace_id") === false){
		}else {
			if($i==0){
				$l_del_workplace_id .= $value;
			}else {
				$l_del_workplace_id .=",".$value;
			}
			$i++;
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
	// Delete対象テーブル名
	$table_name = SCHEMA_NAME.".BASES";
	
	$sql = null;
	$sql .= "delete from ".$table_name." ";
	
	// 更新キーの設定
	$sql .= "where BASE_ID in (".$l_del_workplace_id."); ";
