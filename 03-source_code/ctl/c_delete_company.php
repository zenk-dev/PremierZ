...
/*----------------------------------------------------------------------------
  POST変数取得
  ----------------------------------------------------------------------------*/
	$l_del_company_id = null;
	$i=0;
	// チェックが入ったユーザIDを取得
	foreach($_POST as $key => $value){
		if(mb_strpos($key,"nm_company_id") === false){
		}else {
			if($i==0){
				$l_del_company_id .= $value;
			}else {
				$l_del_company_id .=",".$value;
			}
			$i++;
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
	// Delete対象テーブル名
	$table_name = SCHEMA_NAME.".COMPANIES";
	
	$sql = null;
	$sql .= "delete from ".$table_name." ";
	
	// 更新キーの設定
	$sql .= "where COMPANY_ID in (".$l_del_company_id."); ";
