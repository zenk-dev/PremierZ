...
/*----------------------------------------------------------------------------
  例外定義
  ----------------------------------------------------------------------------*/
	function my_exception_delwstaff(Exception $e){
		//echo "例外が発生しました。";
		//echo $e->getMessage();
	...
	set_exception_handler('my_exception_delwstaff');
...
/*----------------------------------------------------------------------------
  POST変数取得
  ----------------------------------------------------------------------------*/
	$l_del_target_id = null;
	// チェックが入ったIDを取得
	foreach($_POST as $l_key => $l_value){
		if($l_key !== "nm_token_code"){
			if($l_key === 0){
