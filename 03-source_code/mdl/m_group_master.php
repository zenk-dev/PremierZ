...
class m_group_master extends ModelCommon{
/*******************************************************************************
	クラス名：m_group_master.php
	処理概要：グループマスタモデル
*******************************************************************************/
	...
	private $set_view_name	= 'GROUPS_V';		// ビュー名
	private $set_table_name	= 'GROUPS';		// テーブル名
	//※table_nameはModelCommonで定義されています
	
	private $primary_key_col = 'GROUP_ID';	// 主キーの項目
				...
				foreach($value as $item_key => $item_value){
					if(ereg("^[0-9]+$",$item_key) ){
						...
						if($this->htmlspchar_flag == 'Y'){
							...
							// 会社名、グループ名、分類区分、備考は表示用の短縮名を作成する
							if($item_key=="COMPANY_NAME" || $item_key=="GROUP_NAME" || $item_key=="CLASSIFICATION_DIVISION_NAME"){
...
/*============================================================================
	...
	処理概要：INSERT処理を行う
			...
			$p_update_user_id				更新者キー値
  ============================================================================*/
	function execInsert($pr_data, $p_update_user_id = ''){
		...
		$lr_sql = $lc_create_sql->crudSQLString($this->set_table_name, "INSERT", $pr_data , $p_update_user_id);
		...
		if($l_rtcode != RETURN_NOMAL){
			...
			if(mb_strpos($l_rtcode, STATE_DUPLICATE_ENTRY) == 0 && $l_rtcode != RETURN_NOMAL){
				...
				print "入力された「".$this->c_column_info->getColumnNameMS('GROUP_CODE')."」は既に使用されています。"."\n";
			}
			return false;
		}
		return true;
...
/*============================================================================
  ...
  ============================================================================*/
	function execUpdate($pr_data, $p_key_value = '', $p_update_user_id = '', $p_where_phrase = ''){
		...
		require_once('../lib/crudSQL.php');
		$lc_create_sql = new crudSQL();
		$lr_sql = $lc_create_sql->crudSQLString($this->set_table_name, "UPDATE", $pr_data, $p_update_user_id, $p_key_value, $p_where_phrase);
...
/*============================================================================
	...
	引数:
				$p_update_user_id				更新者ユーザーID
  ============================================================================*/
	function insertRecord($p_update_user_id){
		
		foreach($this->r_table_rec as $l_recnum => $lr_record){
			if(!$this->execInsert($lr_record, $p_update_user_id)){
...
/*============================================================================
	更新処理
	処理概要：
				r_table_recにセットされたレコードの更新処理を行う
	引数:
  ============================================================================*/
	function updateRecord($p_update_user_id){
		
		foreach($this->r_table_rec as $l_recnum => $lr_record){
			
			//print(is_null($lr_record[$this->primary_key_col]) || $lr_record[$this->primary_key_col] == '');
			if(is_null($lr_record[$this->primary_key_col]) || $lr_record[$this->primary_key_col] == ''){
				// 主キーの設定が無い場合
				return false;
			}else{
				if(!$this->execUpdate($lr_record, $lr_record[$this->primary_key_col], $p_update_user_id)){
					// エラーとなるレコードが発生した時点で終了
					return false;
				}
			}
		...
		if(count($this->r_table_rec) > 0){
			...
			foreach($this->r_table_rec as $l_key => $lr_data_rec){
				...
				/*--------------------
				  ...
				  --------------------*/
				// グループコード
				if(!is_null($lr_data_rec['GROUP_CODE']) && strlen($lr_data_rec['GROUP_CODE']) > 0){
					if(!preg_match("/^[a-zA-Z0-9\-\_]{1,50}+$/",$lr_data_rec['GROUP_CODE'])){
						$lr_check_res[$l_key]['GROUP_CODE']['STATUS'] = 2;
						$lr_check_res[$l_key]['GROUP_CODE']['MESSAGE'] .= "「".$this->c_column_info->getColumnNameMS('GROUP_CODE')."」は半角英数字と「-」、「_」で入力して下さい。"."\n";
					}
				}
