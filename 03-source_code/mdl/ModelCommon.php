...
/*=============================================================================
	mysql用値の作成
	処理概要
	     mysql_real_escape_stringを掛けた文字列を作成する
	引数:
			$p_value			対象文字列
=============================================================================*/
	function getMysqlEscapedValue($p_value){
		...
		// mysql_real_escape_stringを使用する為に接続を確立する
		if (!mysql_real_escape_string('AAA')){
			$l_link = mysql_connect(DB_SERVER, DB_USER, DB_PASS);
		}
		$l_return_value = mysql_real_escape_string($p_value);
		// mysql_real_escape_stringを使用する為に確立した接続を切断する
		if ($l_link){
			mysql_close($l_link);
		}
