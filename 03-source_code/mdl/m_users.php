		...
		function MailSend(){
			...
			foreach($w_sendtarget as $target){
				...
				if(is_null($target["HOME_MAIL"]) && is_null($target["MOBILE_PHONE_MAIL"])){
					...
					// ユーザ名
					$str_name = ereg_replace("　", "", ereg_replace(" ", "", $target["NAME"]));
					...
					$subject	= ereg_replace(INFORMATION_NAME, $str_name, $subject);
					
					// 本文のユーザ名を置換
					$body		= $_POST["BODY"];
					$body		= ereg_replace(INFORMATION_NAME, $str_name, $body);
					
					// 本文のユーザコードを置換
					$body		= ereg_replace(INFORMATION_USER_CODE, $target["USER_CODE"], $body);
					
					// 本文のパスワードを置換
					$body		= ereg_replace(INFORMATION_PASSWORD, $target["PASSWORD"], $body);
			...
			if ($err_check_code != 1){
				...
				if(count($l_result_rec) == 0){
					...
					if ($authority == AUTH_ADMI || $authority == AUTH_MANG || $authority == AUTH_GEN1 || $authority == AUTH_GEN2) {
						...
						// 本文
						$u_name					=	ereg_replace("　", "", ereg_replace(" ", "", $l_result_rec[1]['NAME']))." 様";
