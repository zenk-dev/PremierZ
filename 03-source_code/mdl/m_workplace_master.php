...
class m_workplace_master extends ModelCommon{
/*******************************************************************************
	クラス名：m_workplace_master.php
	処理概要：作業場所マスタモデル
*******************************************************************************/
	...
	private $set_view_name	= 'BASES_V';	// ビュー名
	private $set_table_name	= 'BASES';		// テーブル名
	//※table_nameはModelCommonで定義されています
	
	private $primary_key_col = 'BASE_ID';	// 主キーの項目
				...
				foreach($value as $item_key => $item_value){
					if(ereg("^[0-9]+$",$item_key) ){
						...
						if($this->htmlspchar_flag == 'Y'){
							...
							// 作業拠点名、会社名、備考は表示用の短縮名を作成する
							if($item_key=="BASE_NAME" || $item_key=="COMPANY_NAME"){
	...
	function execInsert($pr_data, $p_update_user_key = ''){
		...
		if($l_rtcode != RETURN_NOMAL){
			...
			if(mb_strpos($l_rtcode, STATE_DUPLICATE_ENTRY) == 0 && $l_rtcode != RETURN_NOMAL){
				...
				print "入力された「".$this->c_column_info->getColumnNameMS('BASE_CODE')."」は既に使用されています。"."\n";
			}
			return false;
		}
		return true;
		...
		if(count($this->r_table_rec) > 0){
			...
			foreach($this->r_table_rec as $l_key => $lr_data_rec){
				...
				/*--------------------
				  ...
				  --------------------*/
				// 拠点コード
				if(!is_null($lr_data_rec['BASE_CODE']) && strlen($lr_data_rec['BASE_CODE']) > 0){
					if(!preg_match("/^[a-zA-Z0-9\-\_]{1,50}+$/",$lr_data_rec['BASE_CODE'])){
						$lr_check_res[$l_key]['BASE_CODE']['STATUS'] = 2;
						$lr_check_res[$l_key]['BASE_CODE']['MESSAGE'] .= "「".$this->c_column_info->getColumnNameMS('BASE_CODE')."」は半角英数字と「-」、「_」で入力して下さい。"."\n";
					}
				}
				...
				// 電話番号
				if(!is_null($lr_data_rec['TELEPHONE']) && strlen($lr_data_rec['TELEPHONE']) > 0){
					if(!$lc_ccnm->checkPhone($lr_data_rec['TELEPHONE'])){
						$lr_check_res[$l_key]['TELEPHONE']['STATUS'] = 2;
						$lr_check_res[$l_key]['TELEPHONE']['MESSAGE'] .= "「".$this->c_column_info->getColumnNameMS('TELEPHONE')."」は○○-○○-○○形式〔半角数字〕で入力して下さい。(各○○は5桁以内)"."\n";
					}
				}
			}
