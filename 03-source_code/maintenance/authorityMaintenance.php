...
/******************************************************************************
 ファイル名：authorityMaintenance.php
 処理概要  ：権限定義編集画面
 POST受領値：
             nm_token_code              トークン(必須)
             nm_authority_id            権限ID(任意)
******************************************************************************/
...
// ==================================
	...
	// DEFINE_IDを取得
	if(!is_null($_POST['nm_authority_id'])){
		$l_primary_key = $_POST['nm_authority_id'];
	}
...
// ==================================
	...
	// jsファイル
	$ar_js_files	= array(DIR_JS . "jquery.js", "maintenance.js", "mntDm.js", "authorityMaintenance.js");
...
// ==================================
	...
	$lc_authority = new m_authority_master();
	...
	if($l_primary_key != ''){
		// 更新の場合はレコードを取得
		$lr_master_rec = $lc_authority->getRecordAll($l_data_id, $l_primary_key);
		// 更新の場合はDATA_IDをDBの値で表示する
		$l_data_id_disp = htmlspecialchars($lr_master_rec[1]["DATA_ID"]);
		
		// タイトルを更新に設定
		$lc_smarty->assign("maintitle",		"権限定義 - 更新");				// メインタイトル
		...
		// タイトルを新規に設定
		$lc_smarty->assign("maintitle",		"権限定義 - 新規");				// メインタイトル
	...
	$lr_main_data = array(
						...
						array(
							"caption"	=> "AUTHORITY_CODE",
							"type"		=> "text",
							"value"		=> htmlspecialchars($lr_master_rec[1]["AUTHORITY_CODE"]),
							"orgvalue"	=> htmlspecialchars($lr_master_rec[1]["AUTHORITY_CODE"]),
							"remarks"	=> ""
						),
						array(
							"caption"	=> "AUTHORITY_NAME",
							"type"		=> "text",
							"value"		=> htmlspecialchars($lr_master_rec[1]["AUTHORITY_NAME"]),
							"orgvalue"	=> htmlspecialchars($lr_master_rec[1]["AUTHORITY_NAME"]),
							"remarks"	=> ""
						),
						array(
							"caption"	=> "TERMINAL_DIVISION",
							"type"		=> "list",
							"listval"	=> array("PC"=>"PC","Mobile"=>"MO","PC/Mobile"=>"PM","閲覧不可"=>"IM"),
							"orgvalue"	=> htmlspecialchars($lr_master_rec[1]["TERMINAL_DIVISION"]),
							"remarks"	=> "※要COMMON_MASTER登録"
						),
						array(
							"caption"	=> "SCREEN_NAME",
							"type"		=> "text",
							"value"		=> htmlspecialchars($lr_master_rec[1]["SCREEN_NAME"]),
							"orgvalue"	=> htmlspecialchars($lr_master_rec[1]["SCREEN_NAME"]),
							"remarks"	=> ""
						),
						array(
							"caption"	=> "ADMITTED_OPERATION_FLAG",
							"type"		=> "list",
							"listval"	=> array("読込不可"=>"D","読込専用"=>"R","書込可"=>"W"),
							"orgvalue"	=> htmlspecialchars($lr_master_rec[1]["ADMITTED_OPERATION_FLAG"]),
							"remarks"	=> "※要COMMON_MASTER登録"
						),
						array(
							"caption"	=> "REMARKS",
