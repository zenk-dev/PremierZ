...
// 例外定義
// ==================================
	function my_exception_authoritySave(Exception $e){
		echo "例外が発生しました。".$l_txt_rts;
		echo $e->getMessage();
		return;
    }
	set_exception_handler('my_exception_pageUsingConfSave');
...
// ==================================
	// 新規か更新かの判別
	// PAGE_USING_CONF_IDがNULLの場合は新規とみなす
	if($_POST["PAGE_USING_CONF_ID"] == ''){
		$l_mode = "INS";
	}else{
		$l_mode = "UPD";
	}
	...
	$lr_cmc_result = $lc_cmc->checkValue($_POST, 'PAGE_USING_CONF', $l_mode);
...
// ==================================
	...
	$lc_mvld = new m_page_using_conf();
	if($l_mode == "INS"){
		// 新規登録
		$l_retcode = $lc_mvld->insertRecord($l_user_id, $_POST);
	}else{
		// 更新
		$l_retcode = $lc_mvld->updateRecord($l_user_id, $_POST);
	}
