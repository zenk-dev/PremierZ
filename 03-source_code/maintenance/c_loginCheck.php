...
// ==================================
	...
	$rcnt = 0;
	try{
		foreach ($mdb->query($l_select_phrase) as $key => $row) {
	...
	if(count($l_ar_retrec) == 1){
		...
		$lc_sess->setSesseionItem($l_get_key1, $l_ar_retrec[0][$l_get_key1]);
		$lc_sess->setSesseionItem($l_get_key2, $l_ar_retrec[0][$l_get_key2]);
		$lc_sess->setSesseionItem($l_get_key3, $l_ar_retrec[0][$l_get_key3]);
