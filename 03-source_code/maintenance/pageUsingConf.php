...
// 例外定義
// ==================================
	function my_exception_authmain(Exception $e){
		echo "例外が発生しました。";
		echo $e->getMessage();
		return;
    }
	set_exception_handler('my_exception_authmain');
...
// ==================================
	...
	// jsファイル
	$ar_js_files	= array(DIR_JS . "jquery.js", "maintenance.js", "pageUsingConf.js", "mntMain.js");
...
// ==================================
	...
	$lc_smarty->assign("maintitle",		"画面利用管理");				// メインタイトル
	...
	$lc_commonm = new m_common_master();
	$lr_data_id = $lc_commonm->getDATAID();
	if($l_debug_mode==1){print_r($lr_data_id);print "<br>";}
	...
	$lc_db_data		= new m_page_using_conf();
	$lr_db_data		= $lc_db_data->getRecordAll($l_data_id);
	if($l_debug_mode==1){print("<pre>");var_dump($lr_db_data);print("</pre>");}
	
	require_once('../maintenance/h_mntTableSetup.php');
	// mntTableSetupのコンストラクタにレコードを渡すと、明細部を作成してくれます
	$lc_mts = new mntTableSetup($lr_db_data);
	// インデックス項目をコードセットに設定
	$lc_mts->setIndexItemNum(1);				// USE_PAGE
