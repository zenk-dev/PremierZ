...
/******************************************************************************
 ファイル名：c_authorityDel.php
 処理概要  ：権限定義削除
 POST受領値：
             nm_token_code              トークン(必須)
             nm_authority_id            権限ID(必須)
******************************************************************************/
...
// 例外定義
// ==================================
	function my_exception_authorityDel(Exception $e){
		echo "例外が発生しました。";
		echo $e->getMessage();
		return;
    }
	set_exception_handler('my_exception_authorityDel');
...
// ==================================
	...
	if(!is_null($_POST['nm_authority_id'])){
		$l_prekey= $_POST['nm_authority_id'];
	}else{
		// 権限が取得できない場合はエラー
