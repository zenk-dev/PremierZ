...
var $pkey_item_name			= "nm_define_id";				// 定義IDの隠し項目名(変更不可)
var $token_item_name		= "nm_token_code";				// トークンの隠し項目名(変更不可)
var $data_id_item_id		= "id_list_dataid_search";		// DATA_IDの検索項目ID
var $use_page_item_id		= "id_txt_use_page_search";		// 使用ページの検索項目ID
var $this_page				= "valueList.php";				// 自ページのファイル名
var $key_item_colnum		= "3";							// 更新や削除のキーとなる項目の列番号
var $del_post_to			= "c_valueListDel.php";			// 削除処理でPOSTするphpファイル
var $insupd_post_to			= "valueListMaintenance.php";	// 新規登録、更新処理でPOSTするphpファイル
...
function procUpdate($p_checked_index, $p_obj_hidden){
	...
	//alert($("input[name='nm_define_id']").val());
...
function procDelete($p_checked_index){
	...
	$lr_param[$pkey_item_name]	= getKeyValue($p_checked_index);
	
	// 削除処理の起動
	//alert("delete done");
	$.post($del_post_to, $lr_param, callBackFncDel);
	
	alert("DEFINE_ID:"+$lr_param_del[$pkey_item_name]+" を削除しました");
}
// コールバック関数
...
jQuery(function($){
	...
	if($("tr.c_tr_dtl_left")){
		$("tr.c_tr_dtl_left").bind("dblclick", function(){
			...
			// 検索項目にセット
			$("#"+$use_page_item_id).val($l_index_value);
		});
	}
