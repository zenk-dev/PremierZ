...
/******************************************************************************
 ファイル名：loginLogSubView.php
 処理概要  ：ログインログサブ画面
 POST受領値：
             nm_token_code              トークン(必須)
             nm_login_log_id            ログインログID(必須)
******************************************************************************/
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
	// メールログIDを取得
	if(!is_null($_POST['nm_login_log_id'])){
		$l_mail_log_id = $_POST['nm_login_log_id'];
	}else{
		throw new Exception("不正なアクセスです。");
	}
...
// ==================================
	...
	// MAIL_LOG_ID
	$lr_where[$l_where_cnt++] = "LOGIN_LOG_ID = ".$l_mail_log_id;
	...
	// 各項目のアサイン
	$lc_smarty->assign("login_time"				, date("Y-m-d H:i:s",strtotime($lr_db_data[1]['LAST_UPDATE_DATET'])));
	$lc_smarty->assign("used_user_code"			, htmlspecialchars($lr_db_data[1]['USED_USER_CODE']));
	$lc_smarty->assign("used_password"			, htmlspecialchars($lr_db_data[1]['USED_PASSWORD']));
	$lc_smarty->assign("used_company_code"		, htmlspecialchars($lr_db_data[1]['USED_COMPANY_CODE']));
	$lc_smarty->assign("certification_result"	, htmlspecialchars($lr_db_data[1]['CERTIFICATION_RESULT']));
	$lc_smarty->assign("spg_referer"			, htmlspecialchars($lr_db_data[1]['SPG_REFERER']));
	$lc_smarty->assign("spg_remort_addr"		, htmlspecialchars($lr_db_data[1]['SPG_REMORT_ADDR']));
	$lc_smarty->assign("spg_server"				, $lr_db_data[1]['SPG_SERVER']);
	$lc_smarty->assign("remark"					, htmlspecialchars($lr_db_data[1]['REMARK']));
...
// ページ表示
// ==================================
	$lc_smarty->display('LoginLogSubView.tpl');
