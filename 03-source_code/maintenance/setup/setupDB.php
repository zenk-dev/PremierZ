...
/*-----------------------------------------------------------------------------
	AUTHORITY_ID取得処理
 -----------------------------------------------------------------------------*/
	function getAuthorityID($p_data_id, $p_admin_type){
		$l_result_value = "";
		
		$l_auth_sql = "select AUTHORITY_ID from AUTHORITY where AUTHORITY_CODE = '".mysql_real_escape_string($p_admin_type)."' and DATA_ID = ".mysql_real_escape_string($p_data_id).";";
		
		$l_result = mysql_query($l_auth_sql);
		if (!$l_result){
			$l_message = "AUTHORITY_ID取得に失敗しました。";
			mysql_close($l_db_conn);
			print $l_message;
			exit;
		}
		$lr_auth_id = mysql_fetch_row($l_result);
		$l_result_value = $lr_auth_id[0];
...
/*-----------------------------------------------------------------------------
	SQL実行処理
 -----------------------------------------------------------------------------*/
	function execSQL($p_sql, $p_proc_name){
		$l_result = mysql_query($p_sql);
		if (!$l_result){
...
/*-----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/
	// DBサーバ接続
	$l_db_conn = mysql_connect($l_hostname, $l_username, $l_userpass);
	if (!$l_db_conn) {
	...
	// クエリの文字コード変換
	mysql_query("SET NAMES utf8",$l_db_conn);
	
	// データベース選択
	if (!mysql_select_db($l_dbname)) {
	    $l_message = "データベース「".$l_dbname."」は存在しません。";
		mysql_close($l_db_conn);
	    print $l_message;
	    exit;
	}
	...
	try{
		...
		foreach($l_xml_data->sql_file->table_node as $l_table_node){
			...
			execSQL($l_sql, $l_table_node["name"]);
	...
	try{
		...
		foreach($l_xml_data->sql_file->view_node as $l_view_node){
			...
			// SQL実行
			execSQL($l_sql, $l_view_node["name"]);
		}
	...
	try{
		...
		foreach($l_xml_data->sql_file->data_node as $l_data_node){
			...
			if ($l_data_node["name"] == "SYSADMIN"){
				// 管理者のAUTHORITY_IDを取得
				$l_auth_id = getAuthorityID($l_sysadmin_data_id, "SYSADMIN");
			...
			// SQL実行
			execSQL($l_sql, $l_data_node["name"]);
		}
		...
		foreach($l_xml_data->sql_file->data_node as $l_data_node){
			...
			if ($l_data_node["name"] == "USERS"){
				// 管理者のAUTHORITY_IDを取得
				$l_auth_id = getAuthorityID($l_data_id, "ADMIN");
