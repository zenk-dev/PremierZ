...
// ==================================
	...
	foreach($lr_data_rec as $l_key => $lr_valrec){
		...
		if(count($lr_dtlrec) > 0){
			...
			// INSERT処理
			// DBサーバ接続
			$l_db_conn = mysql_connect(DB_SERVER, DB_USER, DB_PASS);
			if (!$l_db_conn) {
			...
			// データベース選択
			if (!mysql_select_db(SCHEMA_NAME)) {
			    $l_message = "データベース「".SCHEMA_NAME."」は存在しません。";
			...
			try{
				...
				foreach($l_xml_data->sql_file->data_node as $l_data_node){
					...
					if ($l_data_node["name"] == "USERS"){
						...
						$l_auth_id = mysql_fetch_row($l_result);
						
						// 登録用のSQLを置換
						$l_sql = preg_replace("/%%admin_auth_id%%/",	$l_auth_id[0], $l_sql);
