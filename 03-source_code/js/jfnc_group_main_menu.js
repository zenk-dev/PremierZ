...
$(function(){
	...
	// ユーザー管理
	$("#id_btn_mainmenu_user").bind("click", function(){
			// 会社名のhiddn項目をクリアする
			$obj_hidden_form_list.find("input[name='nm_comp_name_cond']").val('');
			// グループのhiddn項目をクリアする
			$obj_hidden_form_list.find("input[name='nm_group_name_cond']").val('');
			// 表示中のページ数のhiddn項目を1にする
			$obj_hidden_form_list.find("input[name='nm_show_page']").val("1");
			movePage($obj_hidden_form_main, $user_php_file);
		}
	);
});