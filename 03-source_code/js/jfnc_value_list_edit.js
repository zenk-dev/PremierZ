...
var $ext_list_trgt_item_id;										// リストを起動した項目のID
var $list_div_width				= 294;
var $list_div_height			= 294;
...
function initExtListParam($p_show_page){
	...
	// POST用配列に値を返す項目IDを追加
	$lr_param["value_use_item_id"] = $ext_list_trgt_item_id;
...
function bindExtListButtonEvent(){
	...
	if($("#id_ext_btn_set_value")){
		$("#id_ext_btn_set_value").bind("click", function(){
			...
			// ページのタイトルがユーザー管理かどうか判別
			if(document.title == "ユーザー管理"){
				// セット先が"会社名"か判別
				if($l_set_dest_id == "#id_txt_edit_company_name"){
					// セットされている値を取得
					$l_set_before_value = $($l_set_dest_id).val()
					// 元々セットされている値とこれからセットする値が異なる場合はセットされているグループ名を削除する
					if($l_set_before_value != $l_set_value){
...
/*==============================================================================
  ...
  ============================================================================*/
$(function(){
	$obj_ext_list_param		= $(".c_table_td_search_textval");
	...
	$obj_ext_list_param.bind("dblclick", function(){
		var $l_obj_this = $(this);
		$ext_list_trgt_item_id	= $l_obj_this.attr("id");
		/*
		// ボタンが押された項目の位置を検出
		$ext_list_trgt_top			= parseInt($l_obj_this.offset().top, 10) + 20;
		$ext_list_trgt_left			= parseInt($l_obj_this.offset().left, 10);
		*/
		// 編集画面のリストはポップアップ画面の中央に配置する
		var $l_ext_popup_width	= document.documentElement.clientWidth;
		var $l_ext_popup_height	= document.documentElement.clientHeight;
		$ext_list_trgt_top			= $l_ext_popup_height / 2 - $list_div_height / 2;
		$ext_list_trgt_left			= $l_ext_popup_width / 2 - $list_div_width / 2;
		/*
		alert(
			   ": ext_list_trgt_item_id -> " + $ext_list_trgt_item_id
			+"\n: ext_list_trgt_top -> "     + $ext_list_trgt_top
			+"\n: ext_list_trgt_left -> "    + $ext_list_trgt_left
		);
		*/
		// パラメータ取得
