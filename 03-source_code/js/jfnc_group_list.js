...
var $this_page_file	= "groups_mnt.php";
var	$ar_orgcolor = [];										// 各行の背景色
var $list_key_item_name = "nm_selected_group_id";		// リストのキー項目の隠し項目名
...
function procClickTd($list_td_num){
	...
	// 行番号を元に作業場所IDを取得
	$l_trgt_item_id = "id_groupid_list_menu" + $list_td_num;
	$l_trgt_datakey_id = $("#" + $l_trgt_item_id).val();
	
	// 作業場所IDをhiddn項目に追加
	$obj_hidden_form_list.find("input[name='nm_selected_group_id']").val($l_trgt_datakey_id);
...
function backgroundColorSetup(){
	...
	// 選択中のグループID
	var $l_selected_groupid = $obj_hidden_form_list.find("input[name='" + $list_key_item_name + "']").val();
	
	if($l_trgt_obj){
		$l_trgt_obj.each(function(){
				// グループIDの取得
				$l_row_groupid = $(this).find(".c_td_list_menu_invalid").find("input[type='hidden']").val();
	...
	$("#id_btn_delete").bind("click", function(){
			...
			$(".c_tr_list_menu").each(function(){
				...
				if($("#" + $l_trgt_item_id).attr('checked') == true){
					// 行番号を元に作業場所IDを取得
					$l_trgt_datakey_id = $("#id_groupid_list_menu" + $l_checked_num).val();
					// パラメータ設定
					$lr_param["nm_workplace_id" + $i] = $l_trgt_datakey_id;
					$i++;
				} else{
					$delete_check++;
				}
