...
var $page_logincheck = "../ctl/c_loginCheck.php";	// ログインチェックを行うphpファイル
var $page_nextpage = "../page/mainmenu.php";			// 次の画面
...
function callBackFnc($p_data){
	...
	if($p_data){
		if($p_data == "NG"){
			// 認証失敗の場合
			showMessage($obj_mainform, "認証に失敗しました。入力情報を確認して下さい。");
		}else{
			// mainフォームに値をセット
			$l_html = "<input type=\"hidden\" id=\"" + $id_token_code + "\" name=\"" + $nm_token_code + "\" value=\"" + $p_data + "\"></input>";
			$obj_mainform.append($l_html);
			//alert($l_html);
			// メニュー画面にPOST
			movePage($obj_mainform, $page_nextpage);
		}
	}else{
...
$(function(){
	...
	$("#id_sbt_login").bind("click", function(){
		...
		// ログイン処理起動
		//movePage($obj_mainform, $page_logincheck);
		postPage($obj_mainform, $page_logincheck);
	});
