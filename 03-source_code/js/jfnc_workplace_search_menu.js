...
var $this_page_file_search	= "workplace_mnt.php";
...
$(function(){
	...
	$("#id_btn_input_search").bind("click", function(){
			var $l_comp_name	= "";
			var $l_workplace_name	= "";
		
			// 入力された条件値を取得する
			$l_comp_name=$("#id_txt_cond_comp_name").val();
			$l_workplace_name=$("#id_txt_cond_workplace_name").val();
			
			// 取得した値をhidden項目にセットする
			$obj_hidden_form_list.find("input[name='nm_comp_name_cond']").val($l_comp_name);
			$obj_hidden_form_list.find("input[name='nm_workplace_name_cond']").val($l_workplace_name);
			
			// ページを1に戻す
			$obj_hidden_form_list.find("input[name='nm_show_page']").val("1");
			
			// 自ページにPOSTする
			//alert("会社->"+$l_comp_name+" 作業場所->"+$l_workplace_name);
