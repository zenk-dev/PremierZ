...
var $list_key_item_name = "nm_selected_company_id";			// リストのキー項目の隠し項目名
...
function procClickUser($list_td_num){
	...
	// 行番号を元に会社IDを取得
	$l_trgt_item_id = "id_companyid_list_menu" + $list_td_num;
	$l_trgt_company_id = $("#" + $l_trgt_item_id).val();
	
	// ユーザーIDをhiddn項目に追加
	$obj_hidden_form_list.find("input[name='nm_selected_company_id']").val($l_trgt_company_id);
	// 所属グループ・ユーザー・拠点のタブのページ数を1に設定する
	...
	// 表示するタブを所属グループに変更
	$obj_hidden_form_list.find("input[name='nm_selected_tb']").val("group");
...
function backgroundColorSetup(){
	...
	// 選択中の会社ID
	var $l_selected_compid = $obj_hidden_form_list.find("input[name='" + $list_key_item_name + "']").val();
	if($l_trgt_obj){
		$l_trgt_obj.each(function(){
				// 会社IDの取得
				$l_row_compid = $(this).find(".c_td_list_menu_invalid").find("input[type='hidden']").val();
...
function procValidOnlyCheckBox($p_checkstat){
	...
	// 選択済みIDをクリア
	$obj_hidden_form_list.find("input[name='nm_selected_company_id']").val('');
...
function callBackFncDel($p_data){
	if($p_data){
		//alert($p_data);
		alert("会社削除を実行しました。");
		// 現在のページが一番後ろで表示されている会社を全部削除する場合は、ページ番号を更新
	...
	$("#id_btn_delete").bind("click", function(){
			...
			$(".c_tr_list_menu").each(function(){
				...
				if($("#" + $l_trgt_item_id).attr('checked') == true){
					// 行番号を元にIDを会社IDを取得
					$l_trgt_company_id = $("#id_companyid_list_menu" + $l_checked_num).val();
					// パラメータ設定
					$lr_param["nm_company_id" + $i] = $l_trgt_company_id;
					$i++;
				} else{
					$delete_check++;
				}
			...
			if($i==0){
				...
				// 確認メッセージの表示
				yn=confirm("削除した会社は元に戻すことができませんがよろしいですか？");
				if (yn == true) {
					// パラメータ設定
					$lr_param["nm_token_code"]		= $("input[name='nm_token_code']").val();
					// 削除処理の起動
					$.post("../ctl/c_delete_company.php" ,$lr_param ,callBackFncDel);
				}else {
					alert("会社削除を中止しました。");
				}
			}
