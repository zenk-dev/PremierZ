...
/******************************************************************************
 ファイル名：tasks_edit.php
 処理概要   作業編集画面
 GET受領値：
             bid                            動作モード(必須) insert or update
             nm_token_code                  トークン(必須)
             nm_selected_estimate_id        見積ID(任意)
             nm_selected_work_content_id    作業ID(任意)
             nm_work_cal_yyyy               作業年(任意)
             nm_work_cal_mm                 作業月(任意)
             nm_work_cal_dd                 作業日(任意)
             nm_work_content_id             作業ID(任意)
******************************************************************************/
...
/*----------------------------------------------------------------------------
  例外定義
  ----------------------------------------------------------------------------*/
	function my_exception_taskedit(Exception $e){
		//echo "例外が発生しました。";
		//echo $e->getMessage();
	...
	set_exception_handler('my_exception_taskedit');
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
	...
	// 作業日の取得
	$l_work_cal_dd = $_GET['nm_work_cal_dd'];
	if($l_work_cal_dd != ""){
		$l_work_cal_ymd		= $_GET['nm_work_cal_yyyy']."-".$_GET['nm_work_cal_mm']."-".$_GET['nm_work_cal_dd'];
	}
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
	...
	// 新規の場合
		// 見積ID
		if(!is_null($_GET['nm_selected_estimate_id'])){
			$l_estimate_id = $_GET['nm_selected_estimate_id'];
		}
	}else{
	// 更新の場合
		// 作業ID
		if(!is_null($_GET['nm_work_content_id'])){
			$l_work_content_id = $_GET['nm_work_content_id'];
		}
	}
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
	if($l_proc_mode == OPMODE_INSERT){
		...
		// 検索条件設定
		$lr_cond_dtl = array('WORK_CONTENT_ID = '.$l_work_content_id);
		
		// レコード取得
		$lc_model_class = new m_workcontents('Y', $lr_cond_dtl);
		...
		// 隠し項目用の見積ID
		$l_estimate_id = $lr_update_data[1]['ESTIMATE_ID'];
...
/*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
	...
	// 有効フラグリスト作成
	$l_rec_cnt = 0;
	foreach($lr_validity_flag as $key => $common_val){
		$l_rec_cnt++;
		if($key == htmlspecialchars($lr_update_data[1]['VALIDITY_FLAG'])){
			$ar_validity[$l_rec_cnt] = array("value" => $key, "itemname" => $common_val , "checked" => COLKEY_CHECKED);
		}else{
			$ar_validity[$l_rec_cnt] = array("value" => $key, "itemname" => $common_val);
		}
	}
	
	// 作業ステータス
	$lr_work_status	= $lr_common->getCommonValueRec($l_sess_data_id, "WORK_STATUS");
...
/*-----------------------------------
  ...
  -----------------------------------*/
	...
	$ar_css_files	= array(DIR_CSS."jquery-ui-custom.css", 
							...
							DIR_CSS."v_tasks_edit_block.css", 
							DIR_CSS."example.css", 
...
/*-----------------------------------
  ...
  -----------------------------------*/
	...
	// インクルードするテンプレート
	// ------------------------------
	$lc_smarty->assign("edit_include_tpl"				, "tasks_edit_block.tpl");	// 編集
	...
	// 作業日
	// 新規で且つ作業管理画面で年月日の指定をしている場合はその日付をセットする
	if($l_proc_mode == OPMODE_INSERT and $l_work_cal_ymd != ""){
		$lc_smarty->assign("default_work_date"					, $l_work_cal_ymd);
	}
	
	// 作業コード
	// 新規の場合はデフォルト値をセットする
	if($l_proc_mode == OPMODE_INSERT and $l_work_cal_ymd != ""){
		$lc_smarty->assign("default_work_content_code"			, $l_default_work_content_code);
	}
	...
	// 作業ステータス
	$lc_smarty->assign("ar_work_status"					, $lr_work_status);
	// 新規の場合は、デフォルトのステータスをセットする
	if ($l_proc_mode == OPMODE_INSERT){
		$lc_smarty->assign("default_work_status"		, $l_default_work_status);
	}
